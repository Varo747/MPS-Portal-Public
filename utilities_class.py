...
class Utilities_page(QWidget):
    def __init__(self, account, app):
        ...
        data = rows[1:]
        self.df = pd.DataFrame(data, columns=["Timestamp", "Username", "Idnumber", "Action", "Details", "Success", "Security level"])

        title = QLabel("Reports History (All Users)")
        ...
        unique_users = sorted(self.df["Username"].astype(str).str.strip().unique())
        self.user_filter.addItems(unique_users)
